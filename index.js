module.exports = function (babel) {
    ...
    visitor.Program = function (path, state) {
        ...
        state.set("jsxIdentifier", `${id}.call`.split(".").map(function (name) {
...
function parser(opts) {
    ...
    visitor.JSXElement = {
        exit: function exit(path, file) {
            ...
            if (path.node.children.length) {
                concatExpression = wrapperFn({
                    RESULT: path.node.children
                });
            }
            ...
            if (path.node.start === 0) {
                var wrapperModule = template(`
                    (function (root, factory) {
                        ...
                        module.exports = function(h${components.length ? ', ' + components.join() : ''}) {
                            return RESULT
                        }
                        module.exports.id = '${file.file.opts.filename.replace(/\/.+\//, '').replace('.tpl', '')}';
                        module.exports.components = ${JSON.stringify(components.map(item => item.toLowerCase()))};

                        return module.exports;
                    });
                `);
                callExpr = wrapperModule({
                    RESULT: callExpr
                });
                components = [];
            }
    ...
    function groupAttributes(props) {
        ...
        props.forEach(prop => {
            ...
            if (propNameSplitOnDash.length > 1 && propNameSplitOnDash[0] !== 'data') { // contains -
